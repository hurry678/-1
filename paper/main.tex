...
对于问题一，首先根据量纲一致性将海冰内力写为厚度加权应力通量散度 $\nabla\cdot(h_i\vect\sigma)$。以 VP 各向同性压力项 $P_0/2$ 估计内力尺度，五项力依次为 $1.146\times10^{-2}$、$6.419\times10^{-2}$、$1.281\times10^0$、$1.250\times10^{-1}$ 和 $1.265\times10^{-1}\ \mathrm{N/m^2}$。按科学计数法指数比较，删除同属 $10^{-2}$ 量级的惯性力和海冰科里奥利力，保留拖曳、流变内力和风应力，得到瞬时三力平衡模型 M1。进一步设置完整模型及两个单删项模型进行对照，说明特征量级不能直接等同于场变量误差，M1 的适用范围需限定在缓变、非强剪切海况。
...
对于问题四，将初始冰厚作为函数控制量，建立观测失配与背景先验共同组成的变分泛函；再以 12 个低频余弦模态构造控制空间，在背景场处差分建立切线观测矩阵，将一次增量变分化为 $12\times12$ 正则化法方程。非线性正演线搜索选得步长 $0.75$，总目标由 $536.100$ 降至 $510.802$，下降 $4.719\%$。统一正演得到 48 h 区域平均冰厚 $0.503790\ \mathrm m$、最大冰速 $0.108167\ \mathrm{m/s}$。

对于问题五，按题意把航线终点限定为 24 h 时距目的地 E 最近且 $A<0.6$ 的可通行格，并将逐小时冰密集度场嵌入完整时间展开图。以真实起点作为虚拟节点，采用非负燃油边权的 Dijkstra 算法求得该离散时空图上的全局最优解。目标格为 $(15750,250)\ \mathrm m$，离 E 约 $5.755\ \mathrm{km}$；采用 $1.875\ \mathrm s$ 时间层时，航程 $16.718\ \mathrm{km}$、航时 $0.639583\ \mathrm h$、燃油 $0.491879\ \mathrm t$，全程最大冰密集度为 $0.344227$。
...
\par \textbf{问题五：时变冰情下的 LNG 船最优航线。}以反演参数与初始场生成 24--72 h 逐小时冰密集度场，在 $A>0.6$ 禁行、航速 $V_s=V_{\max}(1-A)$、油耗 $q=q_0+q_1V_s^2$ 的规则下，规划至距目的地最近的 $A<0.6$ 可通行点，并给出离散时空图上的全局最优航线、总航时与总燃油。
...
\item 将初始场反演写成变分泛函，以低频谱控制空间、切线观测矩阵和正则化法方程实现一次增量变分，并通过非线性正演线搜索验收；
\item 将时变禁行区航线规划建模为完整时间展开图，在订正后的最近可通行终点上获得具有明确离散全局最优性的路线。
\end{enumerate}
...
问题四 & 参数估计+优化决策类 & 变分泛函、低频控制空间、增量法方程、线搜索 & 目标下降、条件数、预报正演检查\\
问题五 & 优化决策与控制类 & 完整时间展开图、Dijkstra & 可行性检验、离散最优性、时间层敏感性\\
\bottomrule[1pt]
\end{tabular*}
\label{tab:framework}
\end{table}
...
F_\sigma&=\frac{h_0(P_0/2)}{L_0}=\frac{1.0\times2500}{2.0\times10^4}=0.125000\ \mathrm{N/m^2},\\
...
\end{align}
其中内力尺度按题目参考口径，取 $A=1$ 时的冰强度 $P_0=P^*h_0=5000\ \mathrm{Pa}$，并以 VP 本构中的各向同性压力项 $P_0/2$ 作为代表应力。该估计用于比较科学计数法指数，不代表实际场中的应力散度处处等于 $h_0P_0/(2L_0)$。结果见表 \ref{tab:p1_force} 与图 \ref{fig:p1_force}。
...
厚度加权 VP 内力 $F_\sigma$ & $0.125000$ & $0.9882$\\
...
从表 \ref{tab:p1_force} 与图 \ref{fig:p1_force} 可见，拖曳力为 $10^0$ 量级，VP 内力和风应力为 $10^{-1}$ 量级，惯性力与海冰科里奥利力均为 $10^{-2}$ 量级。因此按题目要求的科学计数法指数比较，删除后两项并保留前三项。这里的判据比较的是指数而不是精确比值；特别是海冰科里奥利力仍约为风应力的 $50.7\%$，故后续还需用同离散模型对照评价删项的结构代价。

\begin{figure}[H]
\centering
\includegraphics[width=0.72\textwidth]{figures/problem1_force_magnitudes.png}
\caption{海冰动量方程五项力的特征量级（拖曳为 $10^0$，内力和风应力为 $10^{-1}$，惯性与海冰科里奥利为 $10^{-2}$）}
\label{fig:p1_force}
\end{figure}

作为补充，两项待删力的特征模长之和相对三项保留力之和为
\begin{equation}
\eta_{IC}=\frac{F_I+F_C}{F_\sigma+F_D+F_W}=4.9358\%.
\label{eq:eta_ic}
\end{equation}
式 \eqref{eq:eta_ic} 只用于补充说明合并尺度，不作为主要删项判据。它不包含力的方向、空间分布和相消信息，更不是力平衡残差或场变量误差；M1 的实际结构代价仍需由共享初边值、空间离散和收敛标准的模型对照确定。

\subsection{模型选择：瞬时三力平衡 M1}

在候选方案层面，本文逐一考察仅删惯性、仅删海冰科里奥利和同时删两项三种简化。根据科学计数法指数，惯性力与海冰科里奥利力同属 $10^{-2}$ 量级，按题意同时删除，题设 M1 为
\begin{equation}
...
由于 $\vect\sigma=\vect\sigma(\nabla\vect U)$，完整 M1 仍是关于冰速场的非线性空间方程，不能把内力项预先视为已知量而一次闭式求解。仅在 Picard 第 $m+1$ 轮冻结上一轮应力时，令
\begin{equation}
\vect R^{(m)}=\nabla\cdot(h_i\vect\sigma^{(m)})+\vect\tau_a,
\end{equation}
才有条件更新关系
\begin{equation}
\vect U^{(m+1)}
=\vect u+\frac{\vect R^{(m)}}{\sqrt{\rho_wC_d|\vect R^{(m)}|}}.
\end{equation}
该式只是冻结内力后的局部子步关系。进一步忽略内力并取静水时，才得到可独立手算的风--拖曳极限 $|\vect U|=0.157102\ \mathrm{m/s}$。
...
中位数 $3.535\%$ 与名义合并尺度 $4.9358\%$ 同量级，说明特征尺度判断能够描述主体区域；但 95\% 分位数已升至 $12.210\%$，最大值更受局部近抵消和小分母放大影响。这一长尾分布解释了为什么“平均量级较小”不能推出“全场结构差异小于 $5\%$”。
...
问题四从少量速度观测反推二维初始冰厚场，是一个典型的高维欠定反问题。初始冰厚并不直接被观测，而是先改变冰强度和应力传递，再通过 6--24 h 的动力过程影响站点冰速；题定网格有 20000 个初值自由度，而可用观测只有 36 个标量分量。本文按题意采用变分数据同化：先将初始冰厚作为函数控制量建立目标泛函，再用低频控制空间把问题化简为可直接求解的增量法方程，最后通过非线性正演线搜索验收候选初值。

\subsection{变分泛函与物理约束}

令 $h_0(x,y)=h_i(x,y,0)$ 为待反演初始场，$h_b$ 为题给背景场，$\mathcal M_{0\to t}$ 表示由 M1、冰厚守恒方程和浅水方程构成的正演算子，$\mathcal H_t$ 表示在观测站点插值冰速的观测算子。定义
\begin{equation}
\begin{aligned}
\mathcal J[h_0]
=&\frac12\sum_{t\in\{6,12,24\}\mathrm h}
\left\|
\mathcal H_t\mathcal M_{0\to t}(h_0)-\vect y_t
\right\|_{\mathsf R_t^{-1}}^2\\
&+\frac{\beta}{2}\|h_0-h_b\|_{L^2}^2
+\frac{\gamma}{2}\|\nabla(h_0-h_b)\|_{L^2}^2.
\end{aligned}
\label{eq:p4_obj}
\end{equation}
第一项拟合 18 组向量速度观测，后两项分别约束背景偏差和空间振荡。内部站点采用 $\sigma=0.01\ \mathrm{m/s}$，固壁矛盾观测点采用 $\sigma=0.10\ \mathrm{m/s}$；该处理只改变误差协方差 $\mathsf R_t$，不修改观测值和无滑移边界。

将离散状态写成 $\vect x_{n+1}=\mathcal M_n(\vect x_n)$，构造拉格朗日函数并对状态取变分，可得伴随递推
\begin{equation}
\vect\lambda_n
=\mathcal M_n'(\vect x_n)^\mathsf T\vect\lambda_{n+1}
+\mathcal H_n^\mathsf T\mathsf R_n^{-1}
(\mathcal H_n\vect x_n-\vect y_n),
\end{equation}
从而初始冰厚方向梯度为
\begin{equation}
\nabla_{h_0}\mathcal J
=\beta(h_0-h_b)-\gamma\Delta(h_0-h_b)
+\mathsf E_h^\mathsf T\vect\lambda_0.
\end{equation}
这给出完整变分问题的求解方向。考虑到维护全耦合伴随模型的工作量，本文在背景场附近采用一次增量变分；它是式 \eqref{eq:p4_obj} 的一阶近似，而不是完整非线性伴随求解的严格等价替代。

\subsection{低频控制空间与增量法方程}

采用低频余弦基表示初值修正：
\begin{equation}
h_0(x,y)
=h_b(x,y)+
\sum_{k=0}^{3}\sum_{l=0}^{2}
c_{kl}\cos\frac{k\pi x}{L_x}\cos\frac{l\pi y}{L_y}.
\end{equation}
控制维数由 20000 降至 12。余弦基满足零法向梯度，低频截断本身构成空间平滑约束。令 $\vect c=\vect0$ 为背景控制量，
\begin{equation}
\vect d=\vect y-\mathcal F(\vect0),
\qquad
\mathsf G=\left.\frac{\partial\mathcal F}{\partial\vect c}\right|_{\vect0},
\end{equation}
其中 $\mathsf G$ 由背景正演与 12 个方向的 $10^{-3}\ \mathrm m$ 前向差分构造。增量泛函为
\begin{equation}
J_{\mathrm{inc}}(\vect c)
=\frac12\|\mathsf G\vect c-\vect d\|_{\mathsf R^{-1}}^2
+\frac12\|\vect c\|_{\mathsf B_c^{-1}}^2,
\end{equation}
一阶变分为零后得到 $12\times12$ 法方程
\begin{equation}
\boxed{
(\mathsf G^\mathsf T\mathsf R^{-1}\mathsf G+\mathsf B_c^{-1})\vect c
=\mathsf G^\mathsf T\mathsf R^{-1}\vect d
}.
\label{eq:p4_normal}
\end{equation}
模态先验标准差取 $\sigma_{kl}=0.05/\sqrt{1+k^2+l^2}\ \mathrm m$，使高频模态受到更强约束。式 \eqref{eq:p4_normal} 可直接用线性代数求解；随后在原非线性正演模型上对步长 $s\in\{0,0.25,0.5,0.75,1\}$ 做线搜索。
...
\midrule
$0$ & $536.100$ & $536.100$ & $0$\\
$0.25$ & $524.145$ & $523.652$ & $0.493$\\
$0.50$ & $514.785$ & $512.813$ & $1.973$\\
$0.75$ & $\mathbf{510.802}$ & $506.363$ & $4.438$\\
$1.00$ & $511.416$ & $503.525$ & $7.890$\\
\bottomrule[1pt]
\end{tabular*}
\label{tab:p4_linesearch}
\end{table}

表 \ref{tab:p4_linesearch} 中 $s=0.75$ 的总目标最小，故选为正式增量变分候选。总目标由 $536.100$ 降至 $510.802$，下降 $4.719\%$；初始冰厚平均绝对修正为 $0.044446\ \mathrm m$，最大修正为 $0.144230\ \mathrm m$，候选初值范围为 $0.130124$--$0.791951\ \mathrm m$。加权切线矩阵的条件数为 $5.582\times10^3$，说明部分厚度模态仍然弱可辨识，背景先验不可删除。全步长的观测项更低，但因先验代价增大，其总目标高于 $s=0.75$；因此选取依据是预先定义的变分泛函，而非单独追求观测残差。

\subsection{48 h 预报结果}

以增量变分候选为初值、$(C_d^*,\alpha^*)=(0.015,0.5)$ 为参数，在 $40\times20$ 网格统一运行至 72 h，并从同一轨迹提取 48 h 场。864/864 个物理步全部完成，72 h 冰体积相对误差为 0，最大冰动量残差 $9.788\times10^{-4}$、最大耦合残差 $9.997\times10^{-6}$、最大冰厚 CFL 为 $0.0901$。

\begin{table}[H]
\centering
\caption{40×20 网格增量变分候选的 48 h 正演统计}
\renewcommand{\arraystretch}{1.4}
\begin{tabular*}{\textwidth}{
    @{\extracolsep{\fill}}
    l
...
指标 & 数值\\
\midrule
48 h 平均冰厚 / m & $0.503790$\\
48 h 最大冰速 / (m/s) & $0.108167$\\
48 h 中心点冰厚 / m & $0.826978$\\
冰体积相对误差 & $0$（机器精度）\\
最小/最大冰厚 / m & $2.638\times10^{-8}$ / $0.941511$\\
\bottomrule[1pt]
\end{tabular*}
\label{tab:p4_48h}
\end{table}

候选初始平均冰厚为 $0.503790\ \mathrm m$，闭域有限体积输运在 48 h 和 72 h 均保持该总体积。图 \ref{fig:p4_48h} 显示，较大冰速集中在西南侧薄冰及冰缘，厚冰主体内部速度较低；这一格局与问题二一致。该结果验证了候选初值的动力一致性和守恒性，但由于控制空间仅含 12 个低频模态，不能解释为原 20000 维函数空间中的唯一全局最优初值。

\begin{figure}[H]
\centering
\includegraphics[width=0.96\textwidth]{figures/problem4_incremental_48h.png}
\caption{增量变分候选驱动的 48 h 冰厚场与冰速场}
\label{fig:p4_48h}
\end{figure}
...
问题五把连续海冰预报转化为船舶可执行的航行决策。经预报检查，题设目的地 E 在 24--72 h 始终位于 $A>0.6$ 的禁行区，因此完整航程不可行。按题目补充要求，本文只规划 LNG 船前段航程：终点固定为 24 h 时距 E 最近且满足 $A<0.6$ 的可通行网格。由于可通行性、航速和燃油边权都随到达时刻变化，采用完整时间展开图保留每一个离散到达时刻，并在该图上求燃油最小路径。

\subsection{模型与离散}

以问题四的增量变分初值和 $(C_d^*,\alpha^*)=(0.015,0.5)$ 运行 M1，生成 0--72 h 逐小时冰密集度场。$40\times20$ 网格上的 864/864 步全部完成，72 h 冰体积相对误差为 0。图 \ref{fig:p5_concentration} 展示 24、48、72 h 的典型场。

\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{figures/problem5_concentration_24_48_72h.png}
\caption{增量变分初值驱动的 24、48、72 h 冰密集度预报（红线为 $A=0.6$）}
\label{fig:p5_concentration}
\end{figure}

在 24 h 场上定义规划终点
\begin{equation}
\boxed{
G=\arg\min_{\vect x}\|\vect x-E\|_2,
\qquad A(\vect x,24\ \mathrm h)<0.6
}.
\end{equation}
在 $40\times20$ 网格上得到 $G=(15750,250)\ \mathrm m$，距连续目的地 $E=(19000,5000)\ \mathrm m$ 为 $5755.432\ \mathrm m$。LNG 船从真实起点 $S=(1000,5000)\ \mathrm m$ 出发；为避免吸附到网格中心造成首段距离偏差，将 S 作为虚拟节点，与周围四个网格中心相连。

最大航速为 $V_{\max}=15\ \mathrm{kt}=7.716\ \mathrm{m/s}$。设航段 $e$ 长度为 $d_e$、进入时刻为 $t_e$、保守代表冰密集度为 $A_e(t_e)$，则
\begin{equation}
\Delta t_e=\frac{d_e}{V_{\max}[1-A_e(t_e)]},
\qquad
\Delta Q_e=\left\{q_0+q_1V_{\max}^2[1-A_e(t_e)]^2\right\}\Delta t_e.
\end{equation}
其中 $q_0=0.5\ \mathrm{t/h}$，若 $V_s$ 以 $\mathrm{m/s}$ 计，则 $q_1$ 的量纲为 $(\mathrm{t/h})/(\mathrm{m/s})^2$。边上冰密集度取出发、到达和中间时刻相邻节点值的最大值；若任一检查值不满足 $A<0.6$，该边禁用。路径优化模型为
\begin{align}
\min_P\quad &Q(P)=\sum_{e\in P}\Delta Q_e,\\
\text{s.t.}\quad
&A_e(t_e)<0.6,\qquad t_e+\Delta t_e\le72\ \mathrm h,\\
&e\text{ 满足 8 邻域移动或等待规则，且路径连续。}
\end{align}

\subsection{算法选择与理由}

状态定义为 $(k,i,j)$，其中 $k$ 是唯一离散时刻，$(i,j)$ 是空间网格。移动边先按物理航速完成航行，再把不足一个时间层的余量记为等待，移动与等待耗油均计入边权；另设显式原地等待边。这样每个状态对应唯一离散时刻，不再把多个精确到达时刻压缩成同一标签。所有边耗油非负，因此 Dijkstra 算法\cite{dijkstra1959,cooke1966}在标签永久确定时给出\textbf{该离散时空图上的全局最优解}。该保证不外推为连续航迹的解析全局最优。

\subsection{航线结果与时间层敏感性}

\begin{table}[H]
\centering
\caption{完整时间展开图的时间层敏感性}
\renewcommand{\arraystretch}{1.4}
\begin{tabular*}{\textwidth}{
    @{\extracolsep{\fill}}
    l
...
$1.875$ & $\mathbf{0.491879}$ & $\mathbf{0.639583}$\\
\bottomrule[1pt]
\end{tabular*}
\label{tab:p5_route}
\end{table}

最后两档燃油差为 $0.50\%$、航时差为 $0.97\%$，说明结果随时间层加密趋于稳定；该档间差异不构成连续时间误差小于 $1\%$ 的严格上界。最终采用 $1.875\ \mathrm s$ 时间层，得到几何航程 $16717.514\ \mathrm m$、物理移动时间 $0.629685\ \mathrm h$、时间层等待 $0.009898\ \mathrm h$、总航时 $0.639583\ \mathrm h$，于 $t=24.639583\ \mathrm h$ 到达，燃油为 $0.491879\ \mathrm t$。路线含 31 个航路点，全程最大冰密集度 $0.344227<0.6$。
...
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}ccccc@{}}
\toprule[1pt]
序号 & 到达时刻/h & 坐标/m & 冰密集度 & 累计燃油/t\\
\midrule
0 & $24.000000$ & $(1000,5000)$ & $0.000538$ & $0$\\
...
\bottomrule[1pt]
\end{tabular*}
\label{tab:p5_waypoints}
\end{table}

\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{figures/problem5_route_strict_time_expanded.png}
\caption{到最近可通行点 $G$ 的离散时空全局最优航线（E 为持续禁行的原目的地）}
\label{fig:p5_route}
\end{figure}

路线从 S 向西南进入低密集度区域，再沿南侧开阔走廊向东到达 G。该结果严格对应题目要求的 LNG 前段航程，不声称船舶已经抵达 E；剩余约 $5.76\ \mathrm{km}$ 需由破冰船接驳、等待冰情改善或采用其他运输方式完成。
...
\item \textbf{模型选择具有对照依据：}问题一通过单删项实验解释双删项差异的来源，问题三用三种优化算法和孪生试验检查参数可辨识性，问题四用非线性线搜索验收增量方向，问题五用三档时间层检查离散敏感性；
\item \textbf{能够报告不可行与负结果：}当 M1 结构差异超过参考线、参数反演落在搜索边界或题设终点不可通行时，模型不强行给出“通过”结论，而是将这些结果用于界定适用范围和提出工程备选方案。
\end{enumerate}

\subsection{稳健性与局限性}

主要局限包括：题定 $200\times100$ 网格的 24 h 全时段尚未完成，$100\times50$ 结果只能作为可完成替代网格；M1 与同离散完整模型的结构差异在当前场景中达到 $7.8\%$--$13.4\%$，该差异会向参数反演和航线规划传播；问题三的最佳已发现参数位于搜索边界，说明参数域或模型结构仍可能限制拟合；问题四只在 12 维低频控制空间中做一次背景场线性化，无法恢复小尺度结构，也不等价于原 20000 维非线性函数空间中的唯一最优解；问题五虽在给定时间展开图上具有全局最优性，但空间网格、边上密集度取值和时间层吸附仍引入离散误差。
...
时空图离散 & 网格中心、8 邻域和时间层吸附 & 路线燃油和等待策略存在离散误差 & 7.5/3.75/1.875 s 三档对照\\
\bottomrule[1pt]
\end{tabular*}
...
稳健性检验覆盖四个维度：时间步由 $300$ s 与 $150$ s 对照；求解器由 24 h 优化前后固定点差异和 6 h 直接/迭代线性求解对照；参数识别由三算法比较与模型一致孪生试验；初始场反演由五档非线性线搜索检查增量步长；航线离散由 7.5、3.75、1.875 s 时间层对照。结果显示，收紧残差几乎不改变 M1--M0 结构差异，求解器优化后的场差异最大为 $0.0228\%$；航线最后两档燃油与航时差分别为 $0.50\%$ 和 $0.97\%$，显示结果随时间层加密趋于稳定，但不构成连续时间误差的严格上界。

\subsection{改进方向}

可扩展方向包括：完成题定 $200\times100$ 全时段并进行至少三层网格收敛研究；将稀疏直接 LU 的 6 h 优势扩展到 24 h 与更高网格验证；开发完整伴随模型并扩大初始场控制空间，以检验一次增量变分的线性化误差；在航线规划中增加 16 邻域或连续航向、进一步缩小时间层，并把破冰船接驳或终点偏差直接纳入联合优化。
...
\par \textbf{问题一}完成海冰动量方程的量纲订正，将内力统一为厚度加权应力通量散度 $\nabla\cdot(h_i\vect\sigma)$；按科学计数法指数，拖曳力为 $10^0$，VP 内力与风应力为 $10^{-1}$，惯性力与海冰科里奥利力为 $10^{-2}$，据此建立瞬时三力平衡 M1。单删项实验表明海冰科里奥利作用对速度方向和冰厚输运影响更明显；严格 D 组中 M1 相对 M0 的四项结构差异为 $7.778\%$--$13.444\%$，说明量级删项不等于场变量误差证明。
...
\par \textbf{问题四}将全场初值反演写成变分泛函，以 12 个低频余弦模态建立增量控制空间，并通过切线观测矩阵化简为 $12\times12$ 正则化法方程。非线性线搜索选得步长 $0.75$，总目标下降 $4.719\%$，最大初值修正 $0.144230\ \mathrm m$。48 h 正演得到平均冰厚 $0.503790\ \mathrm m$、最大冰速 $0.108167\ \mathrm{m/s}$，但结论限于低频控制空间和一次背景线性化。

\par \textbf{问题五}M1 预报下题设目的地持续禁行，按补充要求将终点取为 24 h 时距 E 最近的 $A<0.6$ 网格 $(15750,250)\ \mathrm m$。完整时间展开图上的 Dijkstra 给出离散全局最优路线：航程 $16.718\ \mathrm{km}$、航时 $0.639583\ \mathrm h$、燃油 $0.491879\ \mathrm t$，全程最大冰密集度 $0.344227$；目标格距 E 约 $5.755\ \mathrm{km}$。

\subsection{模型特色}

本文的主要特色是把物理简化、数值求解、反问题和航线决策组织在同一海冰--海水前向内核上。每一次方法选择都设置了对应的验证对象：删项用完整模型与单删项模型解释，求解器用固定点等价和重复计时评价，参数反演用不同优化算法与孪生试验交叉检查，初始场用变分法方程和非线性线搜索验收，航线则先确定最近可通行终点，再在完整时间展开图上给出离散全局最优解。

\subsection{局限性与展望}

主要局限为：题定 $200\times100$ 网格未完成；M1 结构差异超过 $5\%$；参数候选位于边界且真实观测残差较大；初始场反演仅含 12 个低频模态并采用一次线性化；航线的全局最优性只适用于给定空间与时间离散。未来工作应优先补齐高分辨率正演、完整伴随、多尺度控制空间和连续航向验证，再扩展多船协同与随机冰情鲁棒规划。
...
附录 A 汇总各环节数值验证：问题二求解器优化前后残差、守恒、正性、CFL 与源码哈希检查全部通过；$100\times50$ 运行的 288 个物理步最终全部接受，其中 113 步经保护性重试、累计 120 次额外尝试，最大冰残差 $9.98\times10^{-4}$、最大耦合残差 $9.998\times10^{-6}$，冰体积相对误差 $1.5\times10^{-16}$；问题三孪生试验与 $40\times20$ 验证 288/288 步通过；问题四切线矩阵条件数为 $5.582\times10^3$，线搜索总目标下降 $4.719\%$；统一 72 h 正演 864/864 步完成且体积守恒；问题五三个时间层均找到可行离散全局最优路线。详细日志与结构化结果见项目输出目录。
